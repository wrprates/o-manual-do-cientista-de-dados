...
### Como *machine learning* gera valor nos negócios?
...
Um dos desafios ao aplicar K-Means é definir o número ideal de clusters ($k$). O **Método do Cotovelo** (*Elbow Method*) é uma técnica visual que auxilia nesta escolha.

A ideia é calcular a **soma dos quadrados das distâncias intra-cluster** (WSS - *Within-cluster Sum of Squares*) para diferentes valores de $k$ e plotar um gráfico. O ponto onde a curva "dobra" (como um cotovelo) indica um bom número de clusters — adicionar mais clusters após esse ponto traz ganhos marginais decrescentes.

```{r}
# Calculando WSS para diferentes valores de k
wss <- sapply(1:10, function(k) {
  kmeans(iris.new, centers = k, nstart = 10)$tot.withinss
})
...
No gráfico acima, observe que a partir de $k = 3$ a redução do WSS se torna menos acentuada, sugerindo que 3 clusters é uma boa escolha para este conjunto de dados — o que faz sentido, já que sabemos que existem 3 espécies de Iris.
...
Partimos de um dataset com as variáveis originais e chegamos a um dataset reduzido a "**componentes principais**".
...
Veja que os 4 primeiros componentes principais já são responsáveis por 95% da proporção da variância dos dados.
...
> "O modelo de regressão logística é adequado para trabalhar com **dados qualitativos**. Serve para as situações nas quais a **variável dependente (Y) é binária** (assume os valores de 0 e 1) e o valor que **se busca é a probabilidade** ($\pi$) de que a Y seja 1 dado o valor de determinada variável x, que poderá tanto ser binária, numérica ou dividida em categorias." (CHATTERJEE; HADI, 2006; RYAN, 2009). 
...
1. Altere a variável `threshold` para outros valores entre 0 e 1 e rode o código novamente até as métricas pela função `confusionMatrix` do pacote `caret`. Veja como as métricas se comportam. 
...
> Árvores de decisão são fáceis de interpretar, pois fornecem as **regras de negócio** necessárias para classificar os dados e fornecer uma predição.
...
Árvores de decisão podem ficar muito complexas e extensas, caindo no problema de *overfitting*.
...
Iremos utilizar o pacote `ranger`, da linguagem `R `. Este pacote é amplamente utilizado e muito bem implementado. A [documentação](https://www.rdocumentation.org/packages/ranger/versions/0.12.1/topics/ranger) é toda embasada em artigos científicos, o que dá maior credibilidade às funções utilizadas.
...
Os algoritmos de redes neurais não trabalham com fatores (variáveis categóricas) diretamente. O passo que é realizado para que o algoritmo consiga trabalhar é criar `n` variáveis binárias (chamadas de `dummies`). Teremos então que cada `dummy` representará um nível de uma variável categórica. 
...
Entre os modelos treinados, nem sempre é nítida a diferença entre o melhor apenas observando a curva ROC. Sendo assim, o teste de DeLong permite ter um parâmetro objetivo para ajudar na escolha do melhor modelo. O teste possui a hipótese nula ($H_0$) de que a AUC dos testes são iguais.

Veja também que **nem sempre o aumento do poder computacional exigido traz resultados proporcionalmente melhores!**
...
### Validação Cruzada (*Cross-Validation*) {.unnumbered}

Nos exemplos anteriores, dividimos os dados em treino e teste com uma única partição (por exemplo, 70% treino e 30% teste). Porém, essa abordagem possui uma limitação: os resultados podem variar significativamente dependendo de como a divisão foi feita.

A **Validação Cruzada** (*Cross-Validation*) é uma técnica que resolve esse problema ao realizar múltiplas divisões dos dados, treinando e avaliando o modelo em cada uma delas.

**K-Fold Cross-Validation**

A forma mais comum é a **K-Fold Cross-Validation**, que funciona assim:

1. Divide os dados em $k$ partes (*folds*) de tamanho igual
2. Para cada iteração, usa uma parte como teste e as outras $k-1$ como treino
3. Repete o processo $k$ vezes, alternando a parte de teste
4. Calcula a média das métricas de todas as iterações

```{r}
# Exemplo de 5-Fold Cross-Validation com caret
library(caret)

# Definindo o controle de treino com 5-fold CV
ctrl <- trainControl(
  method = "cv",
  number = 5
)

# Treinando modelo de regressão linear com CV no dataset mtcars
set.seed(123)
model_cv <- train(
  mpg ~ .,
  data = mtcars,
...
O resultado mostra as métricas médias (RMSE, R², MAE) calculadas nas 5 iterações de validação cruzada.

**Vantagens da Validação Cruzada:**

- **Estimativa mais robusta**: A média de $k$ avaliações é mais confiável que uma única divisão
- **Uso eficiente dos dados**: Todos os dados são usados tanto para treino quanto para teste
- **Reduz overfitting**: Detecta quando o modelo está se ajustando demais aos dados de treino

**Quando usar:**

- Em conjuntos de dados pequenos ou médios
- Para comparar diferentes modelos ou configurações de hiperparâmetros
- Quando se deseja uma estimativa mais confiável da performance do modelo
